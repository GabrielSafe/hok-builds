...
export default function CountersEditor({ heroId, allHeroes }: Props) {
  ...
  async function addCounter(counter_hero_id: number, type: "strong_against" | "weak_against") {
    const existing = counters.filter(c => c.type === type);
    if (existing.length >= 2) return;
    setSaving(true);
    await fetch(`/api/admin/heroes/${heroId}/counters`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ counter_hero_id, type }),
    });
    await loadCounters();
  ...
  function getFiltered(type: string) {
    ...
    const usedIds = counters.map(c => c.counter_hero_id);
    return allHeroes
    ...
    <div className="bg-dark-700 border border-dark-600 rounded-xl overflow-hidden">
      ...
      <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
        {SECTIONS.map(section => {
          const sectionCounters = counters.filter(c => c.type === section.type);
                    ...
                    <div className="absolute top-full mt-1 w-full bg-dark-600 border border-dark-500 rounded-lg overflow-hidden shadow-xl z-20">
                      {filtered.map(h => (
                        <button
                          key={h.id}
                          type="button"
                          onClick={() => addCounter(h.id, section.type)}
