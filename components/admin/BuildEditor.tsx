...
function ArcanaEditorSection({
  ...
  const usedPerTier = (tier: number) =>
    arcana
      .filter(a => availableArcana.find(av => av.id === a.arcana_id)?.tier === tier)
      ...
      {([1, 2, 3] as const).map(tier => {
        ...
        return (
          <div key={tier} className={`rounded-xl border p-4 mb-4 ${info.border} ${info.bg}`}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex gap-0.5">
                  {Array.from({ length: 10 }).map((_, i) => (
                    <div key={i} className={`w-2.5 h-2.5 rounded-sm ${i < used ? info.bg.replace("/10", "/60") + " border " + info.border : "bg-dark-600 border border-dark-500"}`} />
                  ))}
                </div>
                <span className={`text-[10px] font-bold ${remaining > 0 ? info.text : "text-gray-500"}`}>
                  {used}/10
                </span>
              </div>
            </div>

            {/* Todas as arcanas do tier — sempre visíveis */}
            <div className="flex flex-wrap gap-3">
              {available.map(a => {
                const inBuild = arcana.find(r => r.arcana_id === a.id);
                const currentQty = inBuild?.quantity ?? 0;
                const q = qty[a.id] ?? (inBuild ? currentQty : 1);
                const maxQ = remaining + currentQty; // pode aumentar/diminuir qty da já adicionada

                return (
                  <div key={a.id} className={`flex flex-col items-center gap-1.5 p-2 rounded-xl border transition-colors ${inBuild ? `${info.border} ${info.bg}` : "border-dark-500 bg-dark-700/50"}`}>
                    {/* Ícone */}
                    <div className="relative">
                      <div className={`w-12 h-12 rounded-full border-2 overflow-hidden bg-dark-800 ${inBuild ? info.border : "border-dark-500"}`}>
                        {a.image_url
                          ? <Image src={a.image_url} alt={a.name} width={48} height={48} className="object-cover w-full h-full" />
                          : <div className={`w-full h-full flex items-center justify-center text-sm font-bold ${inBuild ? info.text : "text-gray-500"}`}>{a.name[0]}</div>}
                      </div>
                      {inBuild && (
                        <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black text-dark-900 ${info.bg.replace("/10", "")} border ${info.border}`}
                          style={{ background: tier === 1 ? "#3B82F6" : tier === 2 ? "#22C55E" : "#EF4444" }}>
                          {currentQty}
                        </div>
                      )}
                    </div>
                    ...
                    {/* Nome */}
                    <span className="text-[9px] text-gray-400 text-center max-w-[56px] truncate">{a.name}</span>

                    {/* Controles */}
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => setQty(prev => ({ ...prev, [a.id]: Math.max(1, (prev[a.id] ?? q) - 1) }))}
                        className="w-5 h-5 rounded bg-dark-600 border border-dark-500 flex items-center justify-center text-gray-400 hover:text-white text-xs font-bold">
                        −
                      </button>
                      <span className="w-6 text-center text-xs text-white font-bold">{q}</span>
                      <button
                        onClick={() => setQty(prev => ({ ...prev, [a.id]: Math.min(maxQ || 10, (prev[a.id] ?? q) + 1) }))}
                        className="w-5 h-5 rounded bg-dark-600 border border-dark-500 flex items-center justify-center text-gray-400 hover:text-white text-xs font-bold">
                        +
                      </button>
                    </div>
                    ...
                    {inBuild ? (
                      <div className="flex gap-1">
                        <button
                          onClick={() => onAdd(a.id, q)}
                          className={`px-2 py-1 rounded text-[9px] font-bold transition-colors border ${info.border} ${info.text} hover:bg-white/10`}>
                          Atualizar
                        </button>
                        <button
                          onClick={() => onRemove(inBuild.id)}
                          className="px-2 py-1 rounded text-[9px] font-bold border border-red-500/50 text-red-400 hover:bg-red-500/10 transition-colors">
                          X
                        </button>
                      </div>
                    ) : (
                      <button
                        ...
                        onClick={() => onAdd(a.id, q)}
                        className={`px-3 py-1 rounded text-[9px] font-bold transition-colors border ${remaining > 0 ? `${info.border} ${info.text} hover:bg-white/10` : "border-dark-500 text-gray-600 cursor-not-allowed"}`}>
                        Adicionar
                      </button>
                    )}
                  </div>
            ...
            {available.length === 0 && (
              <p className="text-xs text-gray-600">Nenhuma arcana {info.label.toLowerCase()} cadastrada.</p>
            )}
          </div>
        );
      })}
